...
test('shows authenticated navigation in the root layout', async ({ page }) => {
	...
	await expect(
		header.getByRole('link', { name: headerText.brand }),
	).toHaveAttribute('href', routesFixture.children);
	await expect(
		header.getByRole('link', { name: headerText.navigation.locations }),
	).toHaveAttribute('href', routesFixture.locations);
	await expect(
		header.getByRole('button', { name: headerText.signOut }),
	).toBeVisible();
});

test('redirects an unauthenticated root request to sign in', async ({
	context,
	page,
}) => {
