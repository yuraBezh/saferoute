...
JavaScript `Date` represents a moment in time, so converting a calendar date to `Date` can shift it
by one day depending on the timezone.
...
`2009-12-31` to be stored.

Using local time instead is also unsafe because the result depends on where the code runs.

The same problem applies to `z.coerce.date()`, because it also converts the value to a JavaScript
`Date`.
...
Conversion to `Date` happens only when passing the value to Prisma, via a shared `toDbDate` helper:
...
For example, a date of birth is a calendar date, while a trip pickup time is a real moment in time
and is stored as a UTC timestamp.

Timezone handling for trip scheduling was decided separately, in ADR-0011: `scheduledPickupAt` and
`expiresAt` are stored as `timestamptz`, converted using the pickup location's timezone via `toUtc`
/ `fromUtc`.
