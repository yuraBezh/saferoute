...
Use `CaregiverProfile.status = SUSPENDED` to disable caregiver access. The `CAREGIVER` role describes the user's role in the system and should not be removed as part of the normal suspension workflow. This avoids two competing mechanisms for disabling a caregiver.

If a caregiver profile exists but the role is missing because of manual intervention or legacy data, preserve the profile and deny caregiver access with instructions to contact support.
...
A suspended caregiver retains the `CAREGIVER` role but cannot use caregiver functionality until the profile status changes. Removing the role is not a substitute for suspension.
